...
class PP2HDF5Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, votes_path, hdf5_ori_path, hdf5_seg_path, loader='pil'):
        self.votes = pd.read_csv(votes_path, sep = '\t', header=None)
        self.dataset_ori_path = hdf5_ori_path
        self.dataset_seg_path = hdf5_seg_path
    ...
    def open_hdf5(self):
        self.img_ori_hdf5 = h5py.File(self.dataset_ori_path, 'r')
        self.img_seg_hdf5 = h5py.File(self.dataset_seg_path, 'r')

    def close_hdf5(self):
        self.img_ori_hdf5.close()
        self.img_seg_hdf5.close()
    ...
    def get_image(self, filename):
        img_ori,img_seg = self.load_image(filename)
        img_ori = self.transform(img_ori)
        img_seg = self.transform(img_seg)
        img = torch.cat((img_ori, img_seg), dim=1)
        return img
    ...
    def load_image(self, filename):
        if not hasattr(self, 'img_hdf5'):
            self.open_hdf5()
        if self.loader == 'pil':
            return Image.open(io.BytesIO(np.asarray(self.img_ori_hdf5[filename+'.JPG']))), Image.open(io.BytesIO(np.asarray(self.img_seg_hdf5[filename+'.JPG'])))
        elif self.loader == 'turbojpeg':
            return self.turbojpeg_decoder.decode(self.img_ori_hdf5[filename][()], pixel_format=TJPF_RGB), self.turbojpeg_decoder.decode(self.img_seg_hdf5[filename][()], pixel_format=TJPF_RGB)
