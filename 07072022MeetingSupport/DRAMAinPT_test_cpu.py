...
csv_dir = './pp2/test_csv/'
outputdir = './output_test2attributes/'
hdf5_path = './pp2_images.hdf5'
t = datetime.date.today()
epoch = 1
workers = 0

#for test: csv_dir,outputdir,workers,epoch
...
def get(csv_dir):
    #csv_reader = np.loadtxt(csv_path,delimiter='\t',dtype=str)
    data = {'safe':[],'lively':[]}
    data_train = {'safe':[],'lively':[]}
    data_validation = {'safe':[],'lively':[]}
    data_test = {'safe':[],'lively':[]}
    data_loader_train = {'safe':[],'lively':[]}
    data_loader_test = {'safe':[],'lively':[]}
    data_loader_val = {'safe':[],'lively':[]}
    ...
    num = {'safe':0,'lively':0}
    num_train = {'safe':0,'lively':0}
    num_validation = {'safe':0,'lively':0}
    num_test = {'safe':0,'lively':0}
...
def fusion(img_l,img_r,vgg_extra):
    left_feature = vgg_extra.avgpool(vgg_extra.features(img_l))
    right_feature = vgg_extra.avgpool(vgg_extra.features(img_r))
...
@profile
def main(epoch,image_dir,csv_dir,outputdir,hdf5_path,t,workers,device,batch_size,img_w,img_h,test_iteration):
    #inspect GPU
    ...
    mynet = [MyNet(),MyNet()]
    vgg_extra = (create_vgg19('./vgg_extractor')).to(device)
    ...
    list_val_acc = {'safe':[],'lively':[]}
    list_train_acc = {'safe':[],'lively':[]}
    list_loss = {'safe':[],'lively':[]}
    max_acc = {'safe':0,'lively':0}
    acc_train = {'safe':0,'lively':0}
    ...
    for epo in range(epoch):
        ...
        for iteration, img1img2target in enumerate(data_loader_train_final):
            ...
            for k in range(2):
                ...
                print(img1.device, img2.device, target.device)
                gpu_tracker.track()
                ...
                if iteration%test_iteration==test_iteration-1 or iteration== max_iterations-1:#after 4000 times or all ready train one epoch,start val
                   ...
                   #for times, img1img2target in enumerate(data_loader_val_final):
                    for times, (img1_val, img2_val, target_val) in enumerate(data_loader_val[dic[k]]):
                        ...
                        print(dic[k],img1_val.size(),img2_val.size(),target_val.size())
    ...
    vgg_extra_test.eval()
    mynet_test = [MyNet().to(device),MyNet().to(device)]
...
main(epoch,image_dir,csv_dir,outputdir,hdf5_path,t,workers,device,batch_size,img_w,img_h,test_iteration)