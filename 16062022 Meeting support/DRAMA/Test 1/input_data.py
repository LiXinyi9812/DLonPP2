...
# def get(image_dir,csv_file,batch_size):
# 	csv_reader = csv.reader(open(csv_file))
# 	data = []
# 	x = 0
# 	for row in csv_reader:
# 		if row[2]!='left' and row[2]!='right':
# 			continue
# 		data.append({"dir":[image_dir+row[0]+'.jpg',image_dir+row[1]+'.jpg'],\
# 					"label":0 if row[2]=='left' else 1})
# 	num = len(data)
...
def get_validation(data,image_width, image_height, batch_size,ctx,iteration):
    if iteration>=len(data)//batch_size:
        iteration = iteration%(len(data)//batch_size)
    start = iteration*batch_size
    end = (iteration+1)*batch_size
