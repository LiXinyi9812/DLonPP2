...
[train_wealth,val_wealth,test_wealth]= input_data_test_v0.get_files(image_dir, csv_dir,batch_size)
...
for epoch in range(10):
    ...
    for iteration in range(max_iterations):
        ...
        for k in range(6):
            img_left,img_right,label = input_data_test_v0.get_batch(data_train[k],img_w,img_h, batch_size,ctx,iteration)
            ...
            if iteration%test_iteration==test_iteration-1 or iteration== max_iterations-1:
                ...
                for i in range(times):
                    #left_val,right_val,label = input_data_test_v0.get_validation(data_validation[k],i*batch_size,(i+1)*batch_size,img_w,img_h,ctx)
                    left_val, right_val, label = input_data_test_v0.get_validation(data_validation[k],img_w, img_h, batch_size,ctx,iteration)
...
for k in range(6):
    ...
    for i in range(times):
        if i%20==0:
            print(i,'/',times)
        left_val,right_val,label = input_data_test_v0.get_test(data_test[k],i*batch_size,(i+1)*batch_size,img_w,img_h,ctx)
        val_left_fea = vgg_extra(left_val)
        val_right_fea = vgg_extra(right_val)
        val_label = nd.array(label).copyto(ctx)
        val_left_fea = val_left_fea.reshape((batch_size,-1,49))
